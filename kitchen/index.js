...
const submit = async ()=>{
    ...
    items.forEach((item,i )=>{
        if (i > 0 && item.cells[0].querySelector('input').checked){
            ...
            const contactMethod = cells[8].innerHTML;
            const phone = cells[9].innerHTML;
            const adStatus = cells[10].innerHTML;
            const video = cells[11].innerHTML;
            const obj = {id, title, description, images, price, time, place, 
                         contactMethod, phone, adStatus,  managerName, days_count, video};
            data.push(obj);
            console.log(obj);
        }
